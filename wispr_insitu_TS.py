...
ps = np.arange(0, 360, 0.6)
ts = np.arange(-90, 90, 0.6)
...
def find_PSP_in_WISPR(insitu_time_range, wispr_time, coord='SPP_HCI'):
    ''' find future PSP positions during insitu_time_range in WISPR image imaged at wispr_time'''
    wispr_pos, _ = spice.spkpos('SPP', wispr_time, coord, 'NONE', 'SUN')
    insitu_pos, _ = spice.spkpos('SPP', insitu_time_range, coord, 'NONE', 'SUN')
    sun_pos, _ = spice.spkpos('SUN', insitu_time_range, coord, 'NONE', 'SUN')
    obs_pos = wispr_pos.T / AU
    ...
    for i in range(len(insitu_time_range)):
        ...
        insitu_innerpos_tmp, _ = spice.spkcpt(insitu_pos[:, i] * AU, 'SUN', coord, wispr_time, 'SPP_WISPR_INNER',
                                              'OBSERVER', 'NONE', 'SPP')
        insitu_outerpos_tmp, _ = spice.spkcpt(insitu_pos[:, i] * AU, 'SUN', coord, wispr_time, 'SPP_WISPR_OUTER',
                                              'OBSERVER', 'NONE', 'SPP')
        ...
        if spice.fovray('SPP_WISPR_INNER', insitu_pos[:, i] - obs_pos, coord, 'NONE', 'SPP', wispr_time):
            print('PSP at ' + insitu_utc_tmp + ' Found in WISPR INNER FOV')
        ...
        if spice.fovray('SPP_WISPR_OUTER', insitu_pos[:, i] - obs_pos, coord, 'NONE', 'SPP', wispr_time):
            print('PSP at' + insitu_utc_tmp + 'Found in WISPR OUTER VOF')
...
def Plot_Carrington_map_PSP(wispr_time_str, d, data_path):
    ...
    lats_spp = np.deg2rad(np.linspace(-90, 90, 960 * 2))
    ...
    for lat_spp in lats_spp:
        if l / R < np.cos(lat_spp):
            ...
            if x_inner < 960 and y_inner < 1024 and y_inner > 0 and x_inner > 0:
    ...
    value_msb = np.array(msb) * np.arcsin(d / R)  # /(R**2)
...
def Plot_Jmap_PSP(wispr_time_str, pre_wispr_time_str, data_path):
    ...
    value_msb = (np.array([np.nansum(rd_data[int(x_indexs[i]), int(y_indexs[i] - 15):int(y_indexs[i] + 15)]) for i in
                           range(len(x_indexs))])) / 40
    # +np.array([rd_data[int(x_indexs[i]), int(y_indexs[i])-1] for i in range(len(x_indexs))])\
    # +np.array([rd_data[int(x_indexs[i]), int(y_indexs[i])+1] for i in range(len(x_indexs))])\
    # +np.array([rd_data[int(x_indexs[i]), int(y_indexs[i])-2] for i in range(len(x_indexs))])\
    # +np.array([rd_data[int(x_indexs[i]), int(y_indexs[i])+2] for i in range(len(x_indexs))]))/5
    # print(value_msb)
...
if __name__ == "__main__":
    ...
    # plot_psp_sun_carrington('20210429T134000', '20210429T143000')
    # # plot_psp_sun_hci('20210422T000000', '20210506T000000',13)
    # quit()
    ...
    if plot_Jmap:
        ...
        for i, fname in enumerate(fnamelist):
            ...
            #     pre_wispr_time_str = wispr_time_str
                ...
                pre_wispr_time_str = fnamelist[i - 2][13:28]
    ...
    if plot_Carrmap:
        ...
        for i, fname in enumerate(fnamelist):
            wispr_time_str = fname[13:28]
            wispr_times.append(datetime.strptime(wispr_time_str, '%Y%m%dT%H%M%S'))
            psp_pos, _ = spice.spkpos('SPP', spice.datetime2et(datetime.strptime(wispr_time_str, '%Y%m%dT%H%M%S')),
                                      'SPP_HG', 'NONE', 'SUN')
            radius_psp.append(np.linalg.norm(psp_pos, 2) / Rs)
            print(radius_psp)
        ...
        plt.clim([1e-14, 3e-13])
        plt.xlabel('Carrington Longitude (deg)')
        plt.ylabel('Carrington Latitude (deg)')
        plt.title('Carrington Map (R=' + str(d) + ')')
        # plt.text('Generated by wispr_insitu_TS.py')

        plt.show()

        # np.savez('Carrmap'+str(d),pp,tt,carrmap)

        plt.figure()
        # ts = np.sin(ts)*radius_psp
        ...
        plt.show()
        np.savez('Timemap_' + str(d), time_xx, tt, carrmap)

    # np.savez('Timemap_d='+str(d)+'_0426.npz',time_xx,tt,timemap)
